...
export function Walkthrough({ demo, evaluation, exportFiles }: WalkthroughProps) {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.swarmStrip} role="note" aria-label="Agent groups framing">
        <strong>Agent groups:</strong> persona, committee, research-graph, validation,
        build-planning and evaluation agents collaborate on a shared venture context to produce
      ...
      {/* ───────── Idea ───────── */}
      <Section id="idea" kicker="Step 1" title="The idea" desc="The single thing a founder types in Real Mode. Everything below is generated by the agent group.">
        <div className={styles.card}>
...
function TourBanner() {
  return (
    <div
      ...
      role="note"
      aria-label="Recommended walkthrough"
      data-testid="tour-banner"
    >
      <div className={styles.judgeBannerHead}>
        <span className={styles.judgeBannerBadge}>Product tour</span>
        <span className={styles.judgeBannerTitle}>Recommended walkthrough · seeded demo</span>
      </div>
      <p className={styles.judgeBannerBody}>
        This is the recommended way to explore Foundry end to end. It runs on seeded data and
        needs no sign-in, provider keys, Supabase setup or GitHub access.
      </p>
      <p className={styles.judgeBannerMuted}>
        Real Mode adds BYOK, Google sign-in, a private workspace and real GitHub export for
        deeper testing.
      </p>
    </div>
  );
...
function ResearchSection({ research }: { research: ResearchGraph }) {
  ...
  return (
    <Section
      ...
      desc="Evidence extracted from sources into a typed graph. Key concepts are the highest-leverage nodes; contradictions are surfaced, not hidden."
    >
      <div className={styles.grid4}>
        <Stat value={`${research.stats.nodes}`} label="Nodes" />
        <Stat value={`${research.stats.edges}`} label="Edges" />
        <Stat value={`${research.godNodes.length}`} label="Key concepts" />
        <Stat value={`${research.contradictions?.length ?? 0}`} label="Contradictions" />
      </div>

      <div className={styles.grid2}>
        <div className={styles.card}>
          <p className={styles.cardTitle}>Key concepts (highest centrality)</p>
          <div className={styles.tableWrap}>
            <table className={styles.table}>
