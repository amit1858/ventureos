...
- **Auth — Google sign-in (Supabase Auth) is the recommended path; alpha is a fallback.** The deployed app supports **Google sign-in** via Supabase Auth with an optional `VENTUREOS_ALLOWED_EMAILS` allowlist — appropriate for sharing with a small group of invited testers (~10), where each user gets a private workspace. The **Alpha Workspace** (`/access`, shared `alpha-user` identity) remains as a fallback for hackathon judging and personal testing. It is **not** appropriate for environments where multiple unrelated users will share the same BYOK credentials and ventures. A full per-user OAuth/SSO flow with org-level RBAC is still on the roadmap (Microsoft Entra ID, GitHub OAuth, role-based access control).
